...
- AGENTIC_OS run lineage must stay explicit across docs and schema references: `agentic-os-trace_<ts>.md` -> `kgc_<ts>.md` (`Run`) -> `agentic-os-output_<ts>.md`.
...
- `chatAgentic Graph` canonical acceptance must enforce frontmatter↔body linkage: all body `{{key}}` refs must be declared in YAML frontmatter (base-template allows dotted `runtime.*` refs), and persistence only normalizes `links.self_ref` from the `kgc_yyyymmddhhmmss.md` filename.
