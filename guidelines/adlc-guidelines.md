...
**Reference implementation**: this repository's simplified lean from-0-to-1 operator chain is [`START-WORKFLOW`](../docs/START-WORKFLOW.md) → [`RELEASE-WORKFLOW`](../docs/RELEASE-WORKFLOW.md) → [`DEPLOY-WORKFLOW`](../docs/DEPLOY-WORKFLOW.md): START opens one scoped lane, RELEASE integrates one exact candidate into protected `main`, and DEPLOY releases one exact protected `main` revision. The shared [Agentic OS ADLC](https://github.com/huijoohwee/agentic-os/blob/main/docs/adlc-guidelines.md) remains the execution engine behind those steps; this guideline defines contracts, not another controller.
## Module Index
...
- Keep the operator path externally simple as `START -> RELEASE -> DEPLOY`: start one scoped lane, release one exact candidate into protected `main`, then deploy one exact protected `main` revision; if Dev and Prod are distinct, keep both inside one deploy workflow and name their separate receipts there
...
- Resolve protected-integration authorization from the target's explicit current policy: require eligible independent approval when the policy requires it, or authenticated integration authorization from a policy-permitted authority when the required approval count is zero. Bind the authorization mode, authority identity, immutable review-record locator, exact candidate revision, policy revision, named-check results, decision time, and resulting canonical revision in the Integration Receipt; never represent integration authorization as approval, and return `review-required` when no eligible policy path is available. Advance the frontier only after the resulting protected revision passes its exact-canonical checks; a green task head alone is `canonical-frontier-unverified`. When a declared history-shaping method maps that reviewed candidate to a distinct canonical revision, a topology-bound exact-canonical Integration Receipt may substitute only for an absent or still nonterminal duplicate post-integration canonical-source run. Bind the repository adapter and policy revision; immutable closed review record; reviewed or protected-refreshed candidate revision and tree; protected base revision and tree; resulting canonical revision and tree; declared integration method and exact parent list; integration controller; deferred automatic-integration requester, method, title, and body; actual merger identity and merge time; collaboration claim, review transition, integration receipt, terminal retirement, ledger revision and digest, and operation identity; newest terminal source run, suite, and check chain for the exact candidate plus any declared protected-refresh rollup and remote collaboration-check identity and projection; and a fresh exact read of required-check context, evaluator application, and strictness policy. For every method, require the canonical tree to equal the adapter-declared deterministic transformation of the exact protected base tree plus the admitted reviewed delta; for squash, additionally require parents exactly `[protected base]` and the candidate tree to equal the canonical tree. Associate evidence only through the provider-neutral functional event class `review-record-closed` or `operation-dispatch`; isolate every concrete wire-event mapping in a replaceable reference adapter outside this universal contract, and never infer association from inferred or non-authoritative review association metadata. The newest terminal failure for the same canonical source invalidates the receipt and requires source repair. Keep provider observation read-only and non-mutating and forbid this proof path from synthesizing, updating, rolling up, cancelling, or otherwise mutating checks
