...
- The same Dev-only smoke uses the repository-owned 1,092-byte H.264 fixture at `/demo/media-preview-metadata-ready.mp4` to prove both the adjacent preload and visible video reach `readyState >= HAVE_METADATA` within that 500 ms budget. Its timing artifact has separate structured image and video criteria/results; fixture timing is validation evidence, not production telemetry.
