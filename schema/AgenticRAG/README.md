...
- Knowgrph owns the Agentic Canvas OS checkout repository and immutable revision only in `docs/runtime-readiness-contract.md`. The validated `npm run runtime:docs-dependency:resolve` output feeds Integration, Runtime Verification, and Release after dependency installation; workflow YAML must not copy the SHA or accept a mutable ref.
