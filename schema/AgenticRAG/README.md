...
- FloatingPanel Chat should inject a strict Markdown response contract system prompt; standard chat responses may include one optional `response:` YAML metadata block for parameterized follow-up turns, while `chatKnowgrph` persists only the structured KGC contract. Treat streaming as a Workspace-only concern: write in-progress turns into the active `kgc_*.md` document (Split viewer tail-follow) while Chat UI shows only one final assistant bubble: concise bullets (≤50 words) plus a workspace link to the current `kgc_yyyymmddhhmmss.md`. In `chatKnowgrph` mode, `New Chat` creates and opens a new `kgc_*.md` in Workspace Editor and routes the next turn there. For `chatKnowgrph`, the leading block of `kgc_*.md` should remain the latest standalone parser-valid computing-flow-compatible KGC document, same-session history should move to a trailing section, and validation should require ordered sections plus non-empty `subject/action/goal/solution/request_md/solution_md` before accepting model output; otherwise synthesize a deterministic KGC turn and preserve full prose in `request_md: |` + `solution_md: |`.
- `chatKnowgrph` canonical acceptance must also enforce frontmatter↔body linkage: all body `{{key}}` refs must resolve from top-level YAML frontmatter, and persisted KGC identity (`doc.id`, `doc.created`) should align with the `kgc_yyyymmddhhmmss.md` filename timestamp.
