...
id: agentic-graph-mcp-install-contract
version: 1.1.3
status: implemented
created: 2026-07-10
updated: 2026-07-12
...
# Agentic Graph MCP Install Contract

## Purpose

This document is the canonical install contract for third-party MCP hosts that want to use
Agentic Graph remotely.

If you want the shortest guided reading path across the install contract, release note,
agent-ready context, and MCP overview, start with `docs/documents/agentic-graph-mcp-onboarding-index.md`.
For the source-backed Live Canvas Hero Markdown discovery route behind the landing surface, use
`docs/documents/markdown-convertible-agent-discovery-document.md`.

The boundary is explicit:

- Install `https://airvio.co/agentic-graph/mcp` for public discovery, read-only retrieval, prompts, resources, and inspection.
- Use `https://airvio.co/agentic-graph/control-plane/mcp` only for approval-gated orchestration and live Agentic Canvas OS grammar invocation through `agentic-graph.agentic_canvas_os.docs.invoke`.
...
1. map intent: install `https://airvio.co/agentic-graph/mcp`
2. orchestrate agents: add `https://airvio.co/agentic-graph/control-plane/mcp` only if the host can keep MCP session state across calls
3. if the host cannot do sessioned control-plane MCP, keep public discovery on `/agentic-graph/mcp` and route `/`, `#`, and `@` grammar through an app-owned forwarder
...
`docs/documents/agentic-graph-superagent-harness.md` in the `agentic-graph` repository.

## Host Decision Matrix

| Host situation | Install choice | Why |
|---|---|---|
| Need the fastest working remote MCP install | `https://airvio.co/agentic-graph/mcp` | Lowest-friction read-only discovery surface |
| Need live `/`, `#`, `@` grammar invocation and can preserve MCP session headers | Add `https://airvio.co/agentic-graph/control-plane/mcp` | Grammar is exposed only on the control plane |
| Need grammar lookup but the host cannot manage sessioned MCP | Keep `/agentic-graph/mcp` and use an app-owned forwarder | Preserves install simplicity without pretending one-URL grammar support |
...
|---|---|---|---|
| Public discovery | `https://airvio.co/agentic-graph/mcp` | Canonical install URL for remote MCP hosts | Not exposed here |
| Control plane | `https://airvio.co/agentic-graph/control-plane/mcp` | Approval-gated orchestration surface | Live via `agentic-graph.agentic_canvas_os.docs.invoke` |

## Operator Rule

Keep one canonical operator-facing contract even when the runtime stays split underneath:

- install and discovery stay on `https://airvio.co/agentic-graph/mcp`
- live `/`, `#`, and `@` grammar stays on `https://airvio.co/agentic-graph/control-plane/mcp` or an app-owned forwarder

If a host supports one simple public MCP install, give it `https://airvio.co/agentic-graph/mcp`.

If a host also supports a second Streamable HTTP MCP endpoint with `initialize` plus session-header
handling, add `https://airvio.co/agentic-graph/control-plane/mcp` for live `/`, `#`, and `@` lookup.
...
`https://airvio.co/agentic-graph/mcp` is the public install URL because it is the low-friction,
...
`https://airvio.co/agentic-graph/control-plane/mcp` is a separate MCP surface.

Use it when the remote host needs:

- live Agentic Canvas OS grammar lookup for `/`, `#`, and `@`
- approval-gated orchestration
- control-plane tools such as `agentic-graph.video_remix.run`
...
- Install `https://airvio.co/agentic-graph/mcp`
...
- Add `https://airvio.co/agentic-graph/control-plane/mcp` only when the connector path can maintain the MCP session id across calls and grammar lookup is required
...
- Treat `https://airvio.co/agentic-graph/mcp` as the default MCP server URL
...
- Add the control plane only if the host can perform `initialize`, preserve `mcp-session-id`, and call `agentic-graph.agentic_canvas_os.docs.invoke`
...
- public `tools/list` succeeds on `https://airvio.co/agentic-graph/mcp`
- control-plane `initialize` returns `mcp-session-id`
- control-plane `agentic-graph.agentic_canvas_os.docs.invoke` resolves:
  - `/mcp.capabilities`
  - `#mcp`
  - `@mcp-gateway`
