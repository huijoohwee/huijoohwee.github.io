...
### Canvas Zoom/Fit SSOT (Agentic Graph)

- **Preset SSOT:** `agentic-graph/grph-shared/src/zoom/presets.ts` (capped 16:9 frame + default fill ratio)
- **Consumers:** `agentic-graph/canvas/src/components/GraphCanvas/{fit.ts,simulation.ts}` (fit transforms + seeding/disjoint sizing)
- **Flow renderer:** `agentic-graph/canvas/src/components/FlowCanvas.tsx` applies the same fit/zoom policies while rendering via a native Canvas2D Flow renderer.
...
- **Media renderer + Timeline split:** `agenticOsCanvas2dRenderer: "media"` is preview-only for rich media and playable video-sequence sources. BottomPanel `Timeline` owns transport, playhead, cut/splice, selected-clip nudge/trim/snap/split, scopes, audio mix, Mermaid Gantt writeback, bounded multi-step pinch zoom, rounded scale-domain ruler ticks, bar/lane scale parity, and right-side append workspace; FloatingPanel `Timeline` stays row/list-only. Panel shells must not fork playback state or reach into store internals directly, and timeline zoom chrome must not alter source-backed scrub, drag, or trim math.
...
- **Initialization-file bootstrap parity:** Agentic Graph reads canonical initialization markdown from `huijoohwee/docs`, materializes `/workspace-readme.md`, `/agentic-graph-agentic-video-canvas-demo.md`, and `/agentic-graph-maps-places.md` into the workspace root, and keeps those root-level paths as the stable activation/source-file ids across repos.
- **Initialization-file frontmatter landing:** `workspace-readme.md` lands on 2D D3 + Frontmatter Mode, `agentic-graph-agentic-video-canvas-demo.md` lands on 2D Storyboard + Frontmatter Mode, and `agentic-graph-maps-places.md` lands on Geospatial Mode; renderer/layout helpers must not silently coerce these files back to generic block-layout defaults.
...
### Graph Search Query Filters (Agentic Graph)
...
### UI Event SSOT (Agentic Graph)

- Mode/selection sync events that cross repo boundaries should be centralized in `agentic-graph/grph-shared/src/*/events.ts` to keep payload shapes aligned.

### Floating Panel Lightweight Pattern (Agentic Graph)
...
- FloatingPanel Media is the catalog for uploaded and AI/LLM-generated image/audio/video. Its Upload Media button and the card-field `@ Upload Media` command must share one upload helper and one inventory builder; insertion renders an inline media chip without mutating neighboring text typography. The panel projects the same inventory through native grid cards and three-row list items, not legacy key/type/value columns. Media thumbnails and Storyboard card/reference media previews use shared hover/focus-appearing translucent kind, info, open-link, and Download Media overlays; thumbnail clicks portal a direct shared Rich Media Panel into the previous centered lightbox position and player size without instantiating `MediaLightbox` or replacing insert, rename, delete, or open-link behavior. That direct panel retains the shared native fullscreen action, which follows `fullscreenchange` and switches between Enter and Exit fullscreen states. Visible Previous/Next controls, Left/Up and Right/Down keys, and horizontal touch swipes all reuse one wraparound owner over the currently visible image/video catalog sequence: swipe left advances, swipe right returns, vertical gestures are ignored, and audio is excluded. The sequence owner also selects unique adjacent items for speculative loading: image sources and video metadata preload, while item changes and panel close pause video probes, clear sources, and release obsolete resources. Preload resources stay one-pixel, transparent, pointer-inert, and `aria-hidden`; effect setup assigns their source so React Strict Mode cleanup replay cannot leave a connected preload empty. The Dev-only Rich Media browser smoke measures the preloaded image transition from navigation action to `complete && naturalWidth > 0`, enforces a 500 ms default budget configurable through `AGENTIC_OS_MEDIA_PREVIEW_READY_BUDGET_MS`, and writes `data/outputs/rich-media-catalog-preview-timing.json`; this timing is validation evidence, not production telemetry. Uploaded media surfaces editable media descriptions and `#` metadata fields rather than storage-status copy. Storage responsibilities stay split: R2 bytes, D1 metadata/provenance, KV short-lived access URL cache, Durable Objects room sync notifications.
- The same Dev-only smoke uses the repository-owned 1,092-byte H.264 fixture at `/demo/media-preview-metadata-ready.mp4` to prove both the adjacent preload and visible video reach `readyState >= HAVE_METADATA` within that 500 ms budget. `richMediaBrowserSmokeFixtures.json` is the fixture identity SSOT for path, content type, byte size, SHA-256, 160×90 dimensions, 0.4-second duration, and duration tolerance. Focused coverage hashes the binary, while Chromium asserts and records preload/visible dimensions and finite duration in the structured timing artifact; fixture timing is validation evidence, not production telemetry.
- Every generated timing artifact declares `schema: rich-media-catalog-preview-timing/v1` and must pass the strict Draft 2020-12 schema at `canvas/schemas/rich-media-catalog-preview-timing.v1.schema.json` before the browser smoke can pass. Unknown legacy fields, missing expected/preloaded/visible evidence, and invalid scalar constraints are hard failures; the schema is an evidence-format contract, not a production telemetry API.
...
### Graph Data Table / Multi-dimensional Table (Host Fast Grid) (Agentic Graph)
...
### PDF Workspace (Agentic Graph)

- Local PDF→Markdown workspace artifacts should be written under `.agentic-graph-workspace/` and configured via the Settings key `pdfWorkspaceOutputDirRel`.
- In this repo, `.agentic-graph-workspace/` is moved to `sandbox/.agentic-graph-workspace/` via symlink to avoid storing large artifacts inside `agentic-graph`.
...
- Regression tests should use repo-local fixtures under `agentic-graph/canvas/src/__tests__/fixtures/` and a placeholder URL like `https://example.com/` for deterministic generation.
...
### Runtime sync, chunking, and deploy (Agentic Graph)
...
- Deployments run `pages:build-sync` before `pages:deploy-cloudflare`: sync `canvas/dist` into `huijoohwee/content/agentic-graph`, refresh the managed `/agentic-graph` public-route files, exclude cesium, vendor/mermaid, demo, examples, and large test fixtures, and rely on root `_redirects` + `_headers` for Cloudflare Pages route resolution and caching semantics.
- Responsive parity is part of that deploy gate: when mobile grammar reachability, heavy-runtime intent policy, or touch-first behavior changes, run `npm --prefix canvas run test:smoke:mobile-keyboard:browser`, run `pages:check-sync`, and review `agentic-graph/docs/documents/agentic-graph-feature-map.md` before `pages:deploy-cloudflare`.
- Publish-side mirror notes must not redefine those gates: `huijoohwee.github.io/docs/documents/agentic-graph-mcp-*.md` may index or hand off to the canonical source docs, but responsive, collaboration, storyboard, and storage-route proof logic remains owned by `agentic-graph/docs/documents/agentic-graph-cross-repo-publish-topology.md`.
- Storyboard/Strybldr UI surfaces must normalize the renderer id to `agenticOsCanvas2dRenderer: "storyboard"` (not `"strybldr"`) and keep Strybldr `mediaKind` preserved through import->serialize so release checks remain stable across Dev, generated mirrors, and live Pages.
...
### Chunk-size & mobile responsiveness (Agentic Graph)
...
- Repo hygiene: never commit `.agentic-graph-workspace/**`, `node_modules/**`, or `backups/**` (store workspace artifacts under `sandbox/.agentic-graph-workspace/` via symlink); if large artifacts were committed previously, purge history via `git-filter-repo` and force-push rewritten `main`.

## Pipeline Discipline
- Scope: `/GitHub/{agentic-graph,gympgrph,singabldr}` → import → render.
...
- Keep the Agentic Graph docs map (`schema/AgenticRAG/agentic-graph-documents-map.graph.jsonld`) in sync with `agentic-graph/docs/documents` via `schema/AgenticRAG/sync_map.py --mode check` so schema comments and Markdown docs remain aligned as the import→render pipeline evolves.
...
### Integrations SSOT Sync Directives (Agentic Graph)
...
Recent updates to the Agentic Graph Canvas pipeline ensure seamless rendering of this schema:
