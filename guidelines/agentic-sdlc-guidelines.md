...
- Permit policy-unbounded but operationally bounded concurrent current authorities for disjoint normalized write sets, meaning unlimited concurrent current authorities for disjoint normalized write sets at the policy layer: each repository adapter declares and enforces its current resource, evaluator, and coordination capacity; exactly one current write authority remains permitted per overlapping declared write set, so exactly one current write authority per overlapping declared write set. Authenticate each authority, while local worktrees, leases, PRs, processes, and provider metadata remain replaceable projections. Classify each lane as `canonical`, `overlapping`, `disjoint-attributed`, or `ambiguous`, where an overlapping newcomer is a non-writing waiting successor and undeclared or unparseable future scope is `ambiguous` and cannot create authority
