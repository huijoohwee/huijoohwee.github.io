...
| **C — Dashboard UI lanes** | Agentic Canvas OS PRD + companion | `agentic-graph.agentic_canvas_os.plan` dry-run manifest **implemented** | Canvas Storyboard render of dashboard doc + lane inspectors |
...
| **TCO-Zero** | Live stages opt-in via `AGENTIC_OS_LIVE_CLIENTS` + per-provider secrets; default remains mock/zero-spend | No live call without explicit env + approval token |
...
| **PRD-FO-2**: Live Exa research behind gate | Given `AGENTIC_OS_LIVE_CLIENTS=1`, Exa key, and `paid-model-call` token, when research stage runs, then ≥3 cited sources return and Cost_Log validates | `Verify research-harness live fixture or integration test with mock fetch records ≥3 sources and validateCostLog() passes on emitted log` |
| **PRD-FO-3**: Live storyboard behind gate | Given AI Gateway chat URL + BytePlus model env and approval token, when storyboard runs, then AGENTIC_OS doc emits with nodes == planned shots | `Verify storyboard harness test output parses as agentic-os-computing-flow/v1 with flow.nodes.length == shotCount` |
...
| **PRD-FO-5**: Dashboard Canvas render | Given `agentic-graph.agentic_canvas_os.plan` output, when operator opens dashboard doc in Canvas, then frontmatter-flow projects lanes as Storyboard nodes | `Verify dashboard.agentic-os.md applies through applyChatAgenticOsWorkspaceDocumentToCanvas without duplicate pipeline` |
...
1. Durable Approval_Token persistence on `agentic-graph-mcp` Worker (KV or DO adapter implementing existing store interface).
2. Deploy control plane with `AGENTIC_OS_LIVE_CLIENTS=1` + Exa + AI Gateway chat vars.
3. One operator-captured golden-path run: approved research + storyboard, manifest persisted, demo-pack section populated.
4. Dashboard markdown from `agentic-graph.agentic_canvas_os.plan` opens in Canvas via existing apply path.
...
| Operator approves storyboard | Same token flow | Evidence pack → BytePlus via AI Gateway → AGENTIC_OS doc | Sequential after research | `live-clients.js`, storyboard harness |
...
**Trigger**: `agentic-graph.video_remix.run` with `mode:"live"` and growing `approvals[]`
...
| Executor | Stage harness + live client | Typed stage input | Evidence/AGENTIC_OS/assets/session | ✓ required | mock client when env absent |
...
**Component**: `agentic-graph.agentic_canvas_os.plan` (local MCP)
...
**Decision**: `AGENTIC_OS_LIVE_CLIENTS` + per-provider secrets required for any live call; unset env → deterministic mock → zero provider spend.
...
3. Deploy `agentic-graph-mcp` Worker with KV binding for tokens + live env vars per [`agentic-graph-acos-deploy-runbook.md`](../agentic-graph-acos-deploy-runbook.md) (Cloudflare-only steps; ignore AWS/Vercel sections per ADR-3).
...
node --test mcp/__tests__/research-harness.test.mjs \
  ...
  cloudflare/workers/agentic-graph-mcp/__tests__/tool-registry.test.mjs
...
| `PRD-FO-3 ↔ TAD-storyboard-harness-live ↔ VCC "agentic-os-computing-flow/v1 nodes == shotCount"` |
