...
- Let each repository profile close the history-shaping choice. The Agentic Canvas OS reference adapter is squash-only: it may merge the current protected `origin/main` into the owned task branch to refresh the candidate without rewriting shared history, but it rejects rebasing the canonical frontier, merge-commit integration, and every protected integration method other than squash so `origin/main` remains linear
- Require that adapter's Integration Receipt to bind the reviewed head and tree, the exact protected base, integration method, resulting one-parent canonical integration revision and tree, named checks, authorization evidence, and canonical attribution block. Call that result the `canonical integration commit`; do not use the ambiguous phrase `canonical merge commit`
