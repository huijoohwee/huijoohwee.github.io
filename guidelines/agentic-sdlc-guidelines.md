...
- Bind a release candidate to the source target's exact canonical frontier plus the exact **declared dependency pins**, never to a dependency's live canonical tip: each pinned dependency revision must be an exact fetched revision, an ancestor of that dependency's canonical remote frontier, and protected-check-verified, and the candidate records which binding mode produced each revision. A canonical advance of a dependency repository does **not** invalidate a candidate whose declared pins are unchanged; only a source-frontier advance or a pin change does. Sealing a candidate against a dependency's live tip while other devices may advance it, or invalidating a pinned candidate because an unpinned dependency tip moved, is a `dependency-tip-candidate-coupling` finding
- Admit concurrent multi-device release activity only through the existing target-and-candidate idempotency key and one target-scoped fenced controller; devices coordinate by advancing or consuming pins through protected integration, never by racing seals against a moving dependency tip
