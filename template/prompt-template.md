...
## EXPLORER -> Source Files -> "Initialization-file": (FIX, now unable switch files)
...
MainPanel Integrations, MainPanel Maps, MainPanel Graph Fields, FloatingPanel Props Panel, Widgets (Text, Image, Video), Rich Media Panels (KEEP only Widget-with-floating-toolbar variant, CONSOLIDATE other rich media-/iframe-variants into Widget-with-floating-toolbar variant, after consolidate, REMOVE other non-Widget-with-floating-toolbar variant)-related
...
# ENHANCE E2E Ingestion -> Parsing -> Rendering Pipeline of Widgets (Text, Image, Video), Rich Media Panels, Edges on Canvas 2D Renderer (Flow Editor)) AND (Document Modes (Frontmatter Mode))
- test-validation   `/Users/huijoohwee/Documents/GitHub/knowgrph/knowgrph-video-demo.md` ;
- unpin Widgets -> (FIX, now fly out from original position)
