...
remote `evaluationTime`, that `evaluationTime <= operationTime < expiresAt`, and that the joined
`collaborationReceiptDigest` authorized the exact identity, claim, epoch, fence,
lane, and `mutationSetDigest`. It then joins a valid successor chain to the
latest current disjoint claim; a renewal may change the fence, so validation must
not require its current fence to equal the historical operation fence.

A claim expired at operation time remains invalid after renewal. A valid
operation remains attributable after a subsequent renewal when its historical
authority is valid and the successor chain joins. A malformed, stale, mismatched,
expired-at-operation, or unjoined peer receipt emits `admission-snapshot-stale`
and blocks; prose attribution or global inactivity cannot substitute.

## Report and Decision Contract

The deterministic evaluator emits
`agentic-lane-admission-report/v1` with:
...
- every peer's latest classification and preservation disposition, including
  ...
  progress;
- the accepted canonical cloud claim record, including current claim, lane
  revision, lease epoch, expiry, ledger, fence, and idempotency identities;
- the final protected-ledger observation and digest obtained after local
  provisioning;
...
- Admission Receipt and Preservation Receipt digests when those stages close.
...
8. Compute a current `provisioningPlanDigest` and emit an immutable Admission
   Receipt naming the accepted cloud claim record, predecessor plan, revalidated
   target observation, and exact additive provisioning envelope.
...
13. Derive `authoringAdmission: admitted` only after the current cloud claim,
    atomic candidate result, local lease, final ledger refresh, and Preservation
    Receipt join.
