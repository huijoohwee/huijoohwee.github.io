...
grounding_record: "prd-tad-adr-mvp-gtm-grounding.json"
---
...
The [codebase grounding record](../../guidelines/prd-tad-adr-mvp-gtm-codebase-grounding.md) joins this projection
index to exact source revisions in [one bounded snapshot](./prd-tad-adr-mvp-gtm-grounding.json). CID/RAO/SVO
...
├── agentic-graph-documents-map.graph.jsonld      # Agentic Graph docs/documents ↔ schema surfaces map (sync artifact)
...
## 🔄 Docs Sync (Agentic Graph)

This folder stays in sync with Agentic Graph documentation via a deterministic sync script:

- **Source:** `agentic-graph/docs/documents/**` (repo-relative, recursive)
- **Target artifact:** `schema/AgenticRAG/agentic-graph-documents-map.graph.jsonld`
...
- **Config artifact root:** Agentic Graph repo config artifacts resolve from `data/config/{graphrag,schema,orchestrator,llm-chat}`; schema docs must mirror that source-owned tree and must not preserve alternate root-level config folders.
- Nested document families such as `docs/documents/agentic-graph-api-reference/**`, `docs/documents/agentic-graph-mcp/**`, and future sharded companion folders must be represented in the sync map by their relative document paths, not dropped because they are not top-level files.
- XR invocation and native physics remain source-owned by `agentic-graph/docs/documents/{agentic-graph-xr-invocation-runtime-api,agentic-graph-native-physics-engines-prd-tad}.md`. AgenticRAG mirrors workflow, panel, Canvas, and Source Files boundaries only; it must not register duplicate `/`, `@`, or `#` tokens, reimplement runtime behavior, or admit Rapier code, packages, schemas, fixtures, compatibility claims, services, or runtime dependencies.

### Git Rebase / Conflict Resolution (Cross‑Repo)

- Prefer rebasing when syncing Agentic Graph↔Schema changes to keep one linear history for contract updates.
...
- Cross-repo integration directives are indexed at `agentic-graph/docs/documents/agentic-graph-integrations-ssot-sync-directives.md`.
- Multi-app publish topology keeps `agentic-graph` and `singabldr` as separate Dev SSOT repos, syncs only generated app surfaces into `huijoohwee`, and keeps `airvio.co/agentic-graph` and `airvio.co/singabldr` route ownership isolated.
- Agentic Graph goal-driven refactors use `$GITHUB_ROOT/agentic-graph` as Dev SSOT, `$GITHUB_ROOT/huijoohwee/content/agentic-graph` as generated Prod artifact mirror, and `airvio.co/agentic-graph` as the public Cloudflare route; schema notes mirror upstream docs only after source validation.
- Agentic Graph workspace seeds are authored only under `agentic-graph/docs/workspace-seeds`. `agentic-canvas-os/docs/workspace-seeds` may retain a byte-identical, release-pinned default-storage projection, while publish repositories must not retain an editable `docs/workspace-seeds` copy; duplicate, stale, fallback, renamed, legacy, and conflicting variants are rejected rather than aliased or hidden.
- New release-evidence references belong in immutable `agentic-canvas-os/todo/YYYY-MM/<context>.md` records under the `agentic-canvas-os/docs/TODO.md` contract; planning records confer no release authority. Monthly shards remain immutable history. The migrated 2026-06-29 Dev->Prod->Cloudflare release record is in `agentic-canvas-os/todo/2026-06.md`, anchored by Dev `530462d6`, publish `ec4dfa47`, preview `https://0d3c18ba.joohwee.pages.dev`, live `https://airvio.co/` plus `https://airvio.co/agentic-graph/`, and D1 docs seed `applied=41 conflict=0 rejected=0`.
- Settings-flow provenance remains code-derived and source-owned in `agentic-graph`; generated outputs such as `canvas/public/settings-flow.json` and the publish mirror `huijoohwee/agentic-graph/settings-flow.json` are release artifacts, not downstream hand-edit surfaces.
- Agentic Graph super-agent harness responsive proof is source-owned by `agentic-graph_parser/superagent_responsive.py`, emitted into graph/workspace metadata plus `artifacts/responsive/responsive-proof.json`, and must cover `320x640`, `390x844`, `768x1024`, `1366x768`, and `1920x1080` classes before Cloudflare route parity claims.
- The canonical mobile-first execution queue lives in `agentic-graph/docs/documents/agentic-graph-mobile-first-enhancement-plan.md`; schema notes should mirror only the validated upstream plan and must not create a second mobile checklist.
...
- Publish-side MCP PRD/TAD pages in `huijoohwee.github.io/docs/documents/agentic-graph-mcp-*.md` are mirror notices and index entry points, not downstream contract owners; schema guidance must point edits and release-gate ownership back to the canonical `agentic-graph/docs/documents/**` sources.
- Oversized `agentic-graph/docs/documents` markdown stays split at stable section boundaries into companion files; keep the original filename as the canonical sub-600-line index and keep explicit continuation links to the companions.
- Storage docs follow the same sharding rule: `agentic-graph-storage-document.md` stays the canonical sub-600-line index, companion files carry schema/runtime detail, and the docs map must keep canonical and companion files as separate nodes.
- Agentic Graph storage remains Git-markdown canonical plus browser-local RxDB working state, with optional Cloudflare Worker + D1 shared sync; pulled remote records must re-enter visible `sourceFiles` through the existing runtime path rather than a second import flow.
- Agentic Graph D1 document writes must resolve `(workspace_id, canonical_path)` as the storage identity before insert so seeded docs, Source Files edits, and Share URL publication converge on one row.
...
- Rich-media browser smoke remains a Dev-only verification boundary owned by `agentic-graph`: `canvas/src/features/testing/RichMediaBrowserSmokePage.tsx` plus `npm run test:smoke:rich-media:browser` validate shared `RichMediaPanel` runtime surfaces and emit `data/outputs/rich-media-browser-smoke.png`; schema mirrors should describe that boundary only, not restate it as a deploy artifact.
- `image.to-threejs` is runtime-ready in Dev after focused conversion/lifecycle checks and browser proof cover a PNG Rich Media Panel, runtime-generated JPEG Card, SVG Rich Media Panel, SVG Storyboard Widget, typed fallback, source replacement, and disposal. The source contract remains `agentic-graph/docs/documents/agentic-graph-image-to-threejs-api.md`; schema consumers must not add aliases, external plugin code or dependencies, provider routes, Prod claims, or Cloudflare claims.
- Rich-media preview timing owners remain one affected-CI scope in `agentic-graph`: changes to the v1 schema, validator, fixture manifest, smoke page/runner/verifier, or browser-contract test must automatically select both the timing-schema gate and focused `richMedia.browserSmokeContract` gate.
- Agentic Graph local collaboration keeps exactly one registered worktree per repository on each device and switches task branches in place. The count-only `npm run worktree:check` command reuses the canonical source registry without fetching or starting Dev, and runs first in `ci:integration`, so pre-push and the remote Integration Gate fail closed on redundant linked worktrees.
- Agentic Graph owns the Agentic Canvas OS checkout repository and immutable revision only in `docs/runtime-readiness-contract.md`. The standalone `npm run collaboration:contract:check` auto-discovers every workflow consumer and requires dependency installation, the validated `npm run runtime:docs-dependency:resolve` output, and checkout in order; workflow YAML must not copy the SHA or accept a mutable ref. Its `--json` mode validates against the canonical Draft 2020-12 `schemas/collaboration-runtime-report.v1.schema.json` before emitting `agentic-graph.collaboration-runtime-report/v1`; every report requires a lowercase 40-hex `sourceRevision`, resolved from repository `HEAD` locally and explicitly bound to the pull-request head SHA or push SHA in Integration. Integration uploads and downloads the seven-day `collaboration-contract-report`, captures its machine validation as the separate seven-day `collaboration-validation-result` artifact, then downloads and revalidates that stored envelope against the exact report and expected CI revision before the canonical gate. Every success envelope requires the same `sourceRevision` plus `reportDigest`, the lowercase SHA-256 of the exact report bytes including whitespace and final newline. External consumers can run `npm run --silent collaboration:report:schema` to receive the exact canonical report schema bytes, `npm run --silent collaboration:report:check-schema` to receive the exact canonical `agentic-graph.collaboration-runtime-validation/v1` success/failure envelope schema, `npm run --silent collaboration:report:check-result -- <validation.json|-> [--report <report.json>] [--source-revision <40-hex-sha>]` to validate a stored envelope, prove its exact-byte report and revision pairing, and reject a valid pair replayed from another commit, and `npm run --silent collaboration:report:example` to receive a current schema-valid local report with pull-request coordination marked `not-applicable`; piping that example into `npm run --silent collaboration:report:check -- -` is the canonical path-independent smoke test, while `npm run --silent collaboration:report:check -- --json -` validates every structured success or failure against the envelope schema before writing it, retains stable error codes on stderr, and preserves a nonzero failure exit. AgenticRAG references those upstream contract surfaces and must not copy or redefine either schema, source-revision resolver, digest implementation, example fixtures, validator output, or error taxonomy.
- XR object keyboard motion remains runtime-owned by `agentic-graph`: browser WASD/arrows and `agentic-graph.control_local_animation` reuse one headless movement utility, while `/animation.control #action-path @selected-actor operation=move-object` supplies the canonical invocable grammar. `workflow.jsonld` mirrors the accepted keys, bounded distance/fine-step inputs, stage-clamped resolution, camera isolation, and exact validated Agentic Graph revision; downstream consumers must not reimplement the math, add aliases, or register a second MCP tool.
- XR Motion Control remains browser-runtime-owned by `agentic-graph`: `workflow.jsonld` mirrors the exact merged revision, `agentic-graph.inspect_local_motion_control`, `agentic-graph.control_local_motion_control`, canonical `/motion.control @canvas #pose` grammar, strict open/start/stop input arms, default-disabled Bounding box preference, and privacy boundary. `panels.jsonld` maps Toolbar XR to the reused FloatingPanel Motion Control shell, existing `3D for XR` and `Animation` targets, and sole BottomPanel Timeline transport owner. Downstream consumers must not add aliases, tools, tokens, pose math, detectors, model/runtime code, persisted geometry, validation-specific asset/document/share paths or tokens, or copy external code, assets, schemas, UI, tests, build layouts, or dependencies.
- Storyboard rich-media drag smoke remains a Dev-only boundary owned by `agentic-graph`: browser smoke proves one image and one video `Rich Media Panel` drop without shifting existing storyboard cards or panels, live-route retention proves markdown reapply cleanup, runtime scene seeding prefers the freshest draft graph, and source/target port selectors accept exact ids or workspace-prefixed suffixes.
- Storyboard explicit-output wiring remains source-owned by `agentic-graph`: generated text first publishes to one standalone Rich Media Panel without an inferred edge. Only explicit user edge drags may fan out to one or more Rich Media Panels or downstream Widget Cards; publication must not auto-merge Rich Media targets or overwrite Widget action cards.
- Visual annotation renderer reuse remains Dev-owned by `agentic-graph`: `2D Renderer: Flow Editor`, `2D Renderer: Media`, and `2D Renderer: Storyboard` consume one native dataset runtime for detections, masks, persistent track ids, polygon-zone filters/counts, and dataset save/convert artifacts; external CV projects are reference-only and must not become copied code, package dependencies, or hardcoded validation inputs.
...
- Browser Git and multi-provider file sync remain Dev-runtime-owned by `agentic-graph/docs/documents/agentic-graph-storage-git-file-sync-runtime-api.md`. AgenticRAG mirrors the exact `/git.run @local-git-repository @git-remote #git-remote` and `/file.sync @persisted-cache @file-sync-provider #multi-provider-file-sync` discovery contracts only; it must not add aliases, duplicate WebMCP/stdio tools, expose credentials or provider URLs, substitute filesystem execution for browser IndexedDB, or imply Production/Cloudflare release.
- AGENTIC_OS run lineage must stay explicit across docs and schema references: `agentic-os-trace_<ts>.md` -> `agenticOs_<ts>.md` (`Run`) -> `agentic-os-output_<ts>.md`.
...
- Ingest→parse→render doc sync should treat `agentic-graph-computing-data-flows-import-render-pipeline-document.md` and the computing-flow sample as the canonical fixture/doc pair; companion-file sharding must preserve the original doc path for stable sync-map references.
...
- If conflicts involve `agentic-graph/canvas/src/components/GraphCanvas/scene.ts`, preserve idempotent initialization (no “double-fit” jump) and schema-driven fit (`readFitAllOptions` + `fitAllTransform`).
- Never hand-merge `agentic-graph/canvas/tsconfig.tsbuildinfo`; regenerate via `npm --prefix canvas run check` or `npm --prefix canvas run build`.
...
- Agentic Graph Flowchart client fetches should build `/api/graph`, `?run=`, and `?view=meta` from one shared helper so runtime-meta and data payload paths do not drift.
...
- Mobile/PWA metadata for `/agentic-graph` must come from `agentic-graph/canvas` source and be mirrored by Pages sync, not patched in `content/agentic-graph`.
...
- Installed `/agentic-graph` must precache all hashed `assets/*` chunks, cache same-origin JSON/JSON-LD/webmanifest payloads, and expose Canvas plus Editor manifest shortcuts from source-owned PWA config.
- Installed `/agentic-graph` should publish `data-kg-display-mode` and `data-kg-installed` from one shared PWA runtime helper and surface offline-ready/update-ready signals through the in-app toast path.
- Installed `/agentic-graph` should also recognize `standalone`, `fullscreen`, and `minimal-ui` from the same helper and publish offline/update datasets for CSS-only shell tuning.
...
- Agentic Graph initialization-file bootstrap must treat `huijoohwee/docs` as the source-text SSOT, materialize the canonical 3-file family (`workspace-readme.md`, `agentic-graph-agentic-video-canvas-demo.md`, `agentic-graph-maps-places.md`) into the workspace root, and keep those root-level paths as the stable activation/source-file ids.
- Initialization-file Canvas View switching must stay frontmatter-driven: `workspace-readme.md` -> 2D D3 + Frontmatter Mode, `agentic-graph-agentic-video-canvas-demo.md` -> 2D Storyboard + Frontmatter Mode, `agentic-graph-maps-places.md` -> Geospatial Mode.
...
- Storyboard initialization fixtures such as `agentic-graph-agentic-video-canvas-demo.md` should declare the full explicit frontmatter preset (`schema: "agentic-os-computing-flow/v1"`, `agenticOsCanvasSurfaceMode`, `agenticOsCanvasRenderMode`, `agenticOsCanvas2dRenderer`, `agenticOsDocumentSemanticMode`, `agenticOsFrontmatterModeEnabled`, `agenticOsMultiDimTableModeEnabled`, `agenticOsDocumentStructureBaselineLock`, `kgWorkflowManagerModeEnabled`, `kgAutoSaveEnabled`, `kgAutoSaveDebounceMs`, `kgAutoSaveOn`) so Canvas View switching and runnable workflow persistence remain deterministic. Domain-specific identity belongs in `demo_schema`, never in a competing root graph schema.
...
- `agenticOsCanvas2dRenderer: "animatic"` must reuse the same canonical `flow:` frontmatter graph contract as `agenticOsCanvas2dRenderer: "storyboard"` and keep timeline timing additive under `timeline.beats.*`; do not introduce a parallel animatic-only markdown dialect.
...
- Oversized streaming source docs in `agentic-graph/docs/documents` must keep the original filename as the canonical sub-600 index and move detail into companion markdown files linked by explicit Continuation notes.
- `chatAgentic Graph` canonical acceptance must enforce frontmatter↔body linkage: all body `{{key}}` refs must be declared in YAML frontmatter (base-template allows dotted `runtime.*` refs), and persistence only normalizes `links.self_ref` from the `agenticOs_yyyymmddhhmmss.md` filename.
