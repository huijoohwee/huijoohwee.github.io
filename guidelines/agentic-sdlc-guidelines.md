...
- [Scoped Concurrent Lane Admission](./agentic-sdlc-scoped-lane-admission.md) — additive authoring admission, authoritative write-scope comparison, in-lane scope extension, remote fencing, and preservation proof
...
- [Cloud-Authoritative Collaboration](./agentic-sdlc-cloud-collaboration.md) — provider-neutral multi-device claims, fencing, offline admission, remote runtime-readiness, and durable authority bindings over stable operands with one reachable exit from every producible state
...
- `claim(scope)` admits only an exact clean canonical base, normalized declared scope, authenticated actor, no competing overlap, and a monotonic compare-and-swap transition; failure leaves every existing lane and the requester unchanged. Bind durable authorization to the stable lane identity alone and every volatile operand per operation, prove the claim can record a mutation before work begins, admit declared-scope extension as its own transition, and give every producible authority state one reachable exit; the owning rules and findings are in [Cloud-Authoritative Collaboration](./agentic-sdlc-cloud-collaboration.md) and [Scoped Concurrent Lane Admission](./agentic-sdlc-scoped-lane-admission.md)
- `continue(claim)` revalidates the immutable claim, revision, PR/review identity, epoch, fence, scope, and authenticated authority; recovery from `dormant-preserved` is independent of the expired local lease and never adopts another lane's mutable bytes. Renewal, reclaim, re-anchor, and reconcile never assert the state they restore, decide on claim-layer facts rather than on any projection, and run without an Operator decision while the claim is uncontested; authority gates bind shared mutation only, so a lapsed claim never blocks local recording. One reconcile transition re-derives every projection of a claim at once, divergence is concluded from ancestry and claim ownership rather than from inequality, and authorship is never ownership evidence
...
| Dispatch | `task-model`, `execution-contract`, `tool-permission--blast-radius`, `per-task-budgets`, `autonomous-continuation--interaction-economy`, [Adaptive Goal Completion](./agentic-sdlc-goal-completion.md) |
...
| Escalation | `autonomous-continuation--interaction-economy`, `autonomous-goal-pursuit`, `human-in-the-loop-gates` |
