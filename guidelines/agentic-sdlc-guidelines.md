...
- Let repository policy select the human evidence required for protected integration: either an approving review from an eligible non-author reviewer, or authenticated solo-maintainer authorization when the policy requires zero approvals. Both paths must bind the current pull-request head SHA, actor, policy revision, checks, and decision time. Solo authorization is not self-review and must not create a fabricated approval; it retains the pull-request boundary, required checks, exact-head validation, and all direct-push, force-push, stale-head, and bypass prohibitions
...
- [ ] **Protected integration decision policy-valid**: the exact current head carries either the required eligible non-author approval or, under a zero-approval policy, authenticated solo-maintainer authorization; the pull request, required checks, policy revision, actor, and decision time remain recorded without self-review or bypass
